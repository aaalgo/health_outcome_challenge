...
def construct_2011_name (path):
    ...
    if '2011' in bname:
        return bname.replace('_lds_2011_sample.csv', '_lds_5_2011.csv')
    # hosp_instval_lds_sample.csv
    return bname.replace('_sample.csv', '_5_2011.csv')
...
for path in ['tests/den_saf_lds_5_2012_sample.csv']:
...
for path in glob('tests/*.csv'):
    c = 0
    for key, value in process_file(path):
        if len(key) == 6:               # 2011 data
            keys = KEYS_2012[key]
            if len(keys) == 1:          # uniquely determined
                key = keys[0]
            elif len(keys) == 0:
                assert False
            else:
                print("Ambiguity keys: ", key, "in", keys)
                continue
        else:
            assert len(key) == 9
...
with open('/shared/data/CMS/tests/tests_2011', 'w') as f:
